...
const DesignCanvas: React.FC = () => {
  const { questionnaire, addControl, moveControl } = useQuestionnaire();
    ...
    if (draggedItem && canDropIn('canvas')) {
      try {
        if (draggedItem.isNew) {
          ...
          // Check if we're reordering within the canvas
          const isCanvasReordering = getSourceContainer() === 'canvas';
  ...
  const allControls = [...columnLayouts, ...otherControls];
  ...
  const renderDraggableControl = (control: Control, index: number, isColumn = false) => {
    ...
    const actualIndex = allControls.findIndex(c => c.id === control.id);
    ...
    const controlWrapper = (
      <div 
        ...
        onDragStart={(e) => {
          ...
          setDraggedControlIndex(actualIndex);
          ...
          console.log('[DesignCanvas] Started dragging control:', control.id, control.type, 'at index:', actualIndex);
        ...
        onDragOver={(e) => {
          if (draggedItem && canDropIn('canvas')) {
            ...
            if (draggedControlIndex !== null && 
                (newIndex === draggedControlIndex || newIndex === draggedControlIndex + 1)) {
              return;
            }
